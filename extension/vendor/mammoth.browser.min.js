...
0>end?(tagName=source.substring(tagStart+2).replace(/[\s<].*/,""),errorHandler.error("end tag name: "+tagName+" is not complete:"+config.tagName),end=tagStart+1+tagName.length):tagName.match(/\s</)&&(tagName=tagName.replace(/[\s<].*/,""),errorHandler.error("end tag name: "+tagName+" maybe not complete"),end=tagStart+1+tagName.length);var localNSMap=config.localNSMap,endMatch=config.tagName==tagName,endIgnoreCaseMach=endMatch||config.tagName&&config.tagName.toLowerCase()==tagName.toLowerCase();if(endIgnoreCaseMach){if(domBuilder.endElement(config.uri,config.localName,tagName),localNSMap)for(var prefix in localNSMap)Object.prototype.hasOwnProperty.call(localNSMap,prefix)&&domBuilder.endPrefixMapping(prefix);endMatch||errorHandler.fatalError("end tag name: "+tagName+" is not match the current start tagName:"+config.tagName)}else parseStack.push(config);end++;break;case"?":locator&&position(tagStart),end=parseInstruction(source,tagStart,domBuilder);break;case"!":locator&&position(tagStart),end=parseDCC(source,tagStart,domBuilder,errorHandler);break;default:locator&&position(tagStart);var el=new ElementAttributes,currentNSMap=parseStack[parseStack.length-1].currentNSMap,end=parseElementStartPart(source,tagStart,el,currentNSMap,entityReplacer,errorHandler),len=el.length;if(!el.closed&&fixSelfClosed(source,end,el.tagName,closeMap)&&(el.closed=!0,entityMap.nbsp||errorHandler.warning("unclosed xml attribute")),locator&&len){for(var locator2=copyLocator(locator,{}),i=0;len>i;i++){var a=el[i];position(a.offset),a.locator=copyLocator(locator,{})}domBuilder.locator=locator2,appendElement(el,domBuilder,currentNSMap)&&parseStack.push(el),domBuilder.locator=locator}else appendElement(el,domBuilder,currentNSMap)&&parseStack.push(el);NAMESPACE.isHTML(el.uri)&&!el.closed?end=parseHtmlSpecialContent(source,end,el.tagName,entityReplacer,domBuilder):end++}}catch(e){if(e instanceof ParseError)throw e;errorHandler.error("element parse error: "+e),end=-1}end>start?start=end:appendText(Math.max(tagStart,start)+1)}}function copyLocator(f,t){return t.lineNumber=f.lineNumber,t.columnNumber=f.columnNumber,t}function parseElementStartPart(source,start,el,currentNSMap,entityReplacer,errorHandler){function addAttribute(qname,value,startIndex){el.attributeNames.hasOwnProperty(qname)&&errorHandler.fatalError("Attribute "+qname+" redefined"),el.addValue(qname,value.replace(/[\t\n\r]/g," ").replace(/&#?\w+;/g,entityReplacer),startIndex)}for(var attrName,value,p=++start,s=S_TAG;;){var c=source.charAt(p);switch(c){case"=":if(s===S_ATTR)attrName=source.slice(start,p),s=S_EQ;else{if(s!==S_ATTR_SPACE)throw new Error("attribute equal must after attrName");s=S_EQ}break;case"'":case'"':if(s===S_EQ||s===S_ATTR){if(s===S_ATTR&&(errorHandler.warning('attribute value must after "="'),attrName=source.slice(start,p)),start=p+1,p=source.indexOf(c,start),!(p>0))throw new Error("attribute value no end '"+c+"' match");value=source.slice(start,p),addAttribute(attrName,value,start-1),s=S_ATTR_END}else{if(s!=S_ATTR_NOQUOT_VALUE)throw new Error('attribute value must after "="');value=source.slice(start,p),addAttribute(attrName,value,start),errorHandler.warning('attribute "'+attrName+'" missed start quot('+c+")!!"),start=p+1,s=S_ATTR_END}break;case"/":switch(s){case S_TAG:el.setTagName(source.slice(start,p));case S_ATTR_END:case S_TAG_SPACE:case S_TAG_CLOSE:s=S_TAG_CLOSE,el.closed=!0;case S_ATTR_NOQUOT_VALUE:case S_ATTR:case S_ATTR_SPACE:break;default:throw new Error("attribute invalid close char('/')")}break;case"":return errorHandler.error("unexpected end of input"),s==S_TAG&&el.setTagName(source.slice(start,p)),p;case">":switch(s){case S_TAG:el.setTagName(source.slice(start,p));case S_ATTR_END:case S_TAG_SPACE:case S_TAG_CLOSE:break;case S_ATTR_NOQUOT_VALUE:case S_ATTR:value=source.slice(start,p),"/"===value.slice(-1)&&(el.closed=!0,value=value.slice(0,-1));case S_ATTR_SPACE:s===S_ATTR_SPACE&&(value=attrName),s==S_ATTR_NOQUOT_VALUE?(errorHandler.warning('attribute "'+value+'" missed quot(")!'),addAttribute(attrName,value,start)):(NAMESPACE.isHTML(currentNSMap[""])&&value.match(/^(?:disabled|checked|selected)$/i)||errorHandler.warning('attribute "'+value+'" missed value!! "'+value+'" instead!!'),addAttribute(value,value,start));break;case S_EQ:throw new Error("attribute value missed!!")}return p;case"":c=" ";default:if(" ">=c)switch(s){case S_TAG:el.setTagName(source.slice(start,p)),s=S_TAG_SPACE;break;case S_ATTR:attrName=source.slice(start,p),s=S_ATTR_SPACE;break;case S_ATTR_NOQUOT_VALUE:var value=source.slice(start,p);errorHandler.warning('attribute "'+value+'" missed quot(")!!'),addAttribute(attrName,value,start);case S_ATTR_END:s=S_TAG_SPACE}else switch(s){case S_ATTR_SPACE:el.tagName;NAMESPACE.isHTML(currentNSMap[""])&&attrName.match(/^(?:disabled|checked|selected)$/i)||errorHandler.warning('attribute "'+attrName+'" missed value!! "'+attrName+'" instead2!!'),addAttribute(attrName,attrName,start),start=p,s=S_ATTR;break;case S_ATTR_END:errorHandler.warning('attribute space is required"'+attrName+'"!!');case S_TAG_SPACE:s=S_ATTR,start=p;break;case S_EQ:s=S_ATTR_NOQUOT_VALUE,start=p;break;case S_TAG_CLOSE:throw new Error("elements closed character '/' and '>' must be connected to")}}p++}}function appendElement(el,domBuilder,currentNSMap){for(var tagName=el.tagName,localNSMap=null,i=el.length;i--;){var a=el[i],qName=a.qName,value=a.value,nsp=qName.indexOf(":");if(nsp>0)var prefix=a.prefix=qName.slice(0,nsp),localName=qName.slice(nsp+1),nsPrefix="xmlns"===prefix&&localName;else localName=qName,prefix=null,nsPrefix="xmlns"===qName&&"";a.localName=localName,nsPrefix!==!1&&(null==localNSMap&&(localNSMap={},_copy(currentNSMap,currentNSMap={})),currentNSMap[nsPrefix]=localNSMap[nsPrefix]=value,a.uri=NAMESPACE.XMLNS,domBuilder.startPrefixMapping(nsPrefix,value))}for(var i=el.length;i--;){a=el[i];var prefix=a.prefix;prefix&&("xml"===prefix&&(a.uri=NAMESPACE.XML),"xmlns"!==prefix&&(a.uri=currentNSMap[prefix||""]))}var nsp=tagName.indexOf(":");nsp>0?(prefix=el.prefix=tagName.slice(0,nsp),localName=el.localName=tagName.slice(nsp+1)):(prefix=null,localName=el.localName=tagName);var ns=el.uri=currentNSMap[prefix||""];if(domBuilder.startElement(ns,localName,tagName,el),!el.closed)return el.currentNSMap=currentNSMap,el.localNSMap=localNSMap,!0;if(domBuilder.endElement(ns,localName,tagName),localNSMap)for(prefix in localNSMap)Object.prototype.hasOwnProperty.call(localNSMap,prefix)&&domBuilder.endPrefixMapping(prefix)}function parseHtmlSpecialContent(source,elStartEnd,tagName,entityReplacer,domBuilder){if(/^(?:script|textarea)$/i.test(tagName)){var elEndStart=source.indexOf("</"+tagName+">",elStartEnd),text=source.substring(elStartEnd+1,elEndS-\u2FEF\u3001-\uD7FF\uF900-\uFDCFarCodeAt(i+1)]<<12|revLookup[b64.eDrainedQueues=!1,this._trampolineEnabled=!0;var self=this;this.drainQueues=function(){self._drainQueues()},this._schedule=schedule}function AsyncInvokeLater(fn,receiver,arg){this._lateQueue.pe){for(;queue.length()>0;){var fnreate;if(cr){var callerCache=cr(n.replace("propertyName",propertyNRemainingToCancel<=0},Promise.protryCatch(cb).call(boundTo,e)}else if(util.isObject(e)){for(var ken(){var ctx=this._peekContext();c("\n")),util.notEnumerableProp(ertLastIndex],commonRootMeetPoint=-var stack=error.stack,message=errt.fileName,lastIndex=result.line;ionIsNotified(),fireRejectionEventtachExtraTrace=longStackTracesAt].apply(null,arguments))}catch(e)ngth)){for(var nodes=[],stackToInch(e){hasStackAfterThrow="stack"ivoid 0,thrower,void 0,{reason:reason},void 0);var _reason=arguments[1],handler=function(){throw _reason};return this.caught(reason,handler)},Promise.prototype.catchReturn=function(value){if(arguments.length<=1)return value instanceof Promise&&value.suppresWarning=subError("Warning","warni,propertyIsWritable:function(obj,eturnedNonUndefined();var maybePrnternal._setOnCancel(this)}else{v_promise},PromiseSpawn.prototype.n\n\n    See http://goo.gl/MqrFmX                    \n                     \n            };                                       return code=code.replace(/\[TheNa_queue=[],async.invoke(this._asynmaybePromise._value()}values[indeallback(value.e,!1):this._resolvetion"==typeof nodeback){var adaptuire("./thenables")(Promise,INTER ?handler:"function"==typeof handlct,promise,receiver,null)},Promisl.ensureErrorObject(reason),hasStack=trace===reason;if(!hasStack&ancel()):"function"==typeof handle(promise,handler,receiver,value)(Promise,PromiseArray,apiRejectio0!==(16777216&bitField)?this._rej_values;if(this._cancel(),values }nts","caller","callee","prototype                 \n        ".replmaybeWrapAsError","nodebackForProstValue=tryConvertToPromise(promises);return isObject(castValue)?rg){var length=this.length()+3;ifue),initialValue._attachCancellatll},ReductionPromiseArray.prototyssList.toggle("foo"),toggleScheduits(SomePromiseArray,PromiseArray0))},Promise.some=function(promis,Promise.prototype.reason=functioe){return delay(+this).thenReturnces.length,ret=new Promise(INTERNAL);return iterator(),ret}function Disposer(data,promise,context){this._data=data,this._promise=promise,this._context=context}function FunctionDisposer(fn,promise,context){this.constructor$(fn,promise,context)}function maybce);maybePromise instanceof Promireturn dispose(resources,inspection)});return resources.promise=psAssignmentAndStaticMethods)retur;i<excludedPrototypes.length;++i)dRange,toString:safeToString,canAvalue,encodingOrOffset):fromObject(that,value)}function assertSizBh+byteOffset),byteOffset>=buffer.buf,string,offset,length){return r i=start;end>i;++i)ret+=String.funits){units=units||1/0;for(var c,"undefined"!=typeof Symbol&&Symbol.species&&Buffer[Symbol.specie,length):slowToString.apply(this, buffer bounds");encoding||(encodurn noAssert||checkOffset(offset,otype.readDoubleBE=function(offsereturn value=+value,offset=0|offset,noAssert||checkInt(this,value,offset,1,255,0),Buffer.TYPED_ARRAY_SUPPORT||(value=Math.floor(value)),this[offset]=255&value,offset+1},Buffer.prototype.writeUInt16LE=function(value,offset,noAssert){return value=+value,offsert){return value=+value,offset=0n 0;if(0>targetStart)throw new Ra","Dingbat dec":"35","Dingbat hexe dec":"56","Unicode hex":"38"},{"},{"Typeface name":"Symbol","Dine":"Symbol","Dingbat dec":"99","Dingbat hex":"63","Unicode dec":"name":"Symbol","Dingbat dec":"120hex":"2191"},{"Typeface name":"SyC2","Unicode dec":"8476","Unicodebol","Dingbat dec":"215","Dingbatode hex":"23A3"},{"Typeface name","Dingbat hex":"21","Unicode dec"at dec":"53","Dingbat hex":"35","73","Dingbat hex":"49","Unicode d"Dingbat hex":"51","Unicode dec":at dec":"101","Dingbat hex":"65",ace name":"Webdings","Dingbat dec":"121","Dingbat hex":"79","Unicode dec":"8854","Unicode hex":"2296"},{"Typeface name":"Webdings","Dingbat dec":"122","Dingbat hex":"7A","Unicode dec":"128685","Unicode hex":"1F6AD"},{"Typeface name":"Webdings","Dingbat decUnicode hex":"1F3CE"},{"Typeface t hex":"A1","Unicode dec":"128368"Webdings","Dingbat dec":"181","Dnicode hex":"1F581"},{"Typeface nhex":"DC","Unicode dec":"127785",s","Dingbat dec":"240","Dingbat h{"Typeface name":"Wingdings","Din"128430","Unicode hex":"1F5AE"},{"Unicode dec":"128528","Unicode hex":"5F","Unicode dec":"9801","Un,"Dingbat hex":"73","Unicode dec"dings","Dingbat dec":"135","Dingb"Dingbat dec":"143","Dingbat hex":"8F","Unicode dec":"10125","Uni86"},{"Typeface name":"Wingdings"nicode dec":"10032","Unicode hex","Dingbat dec":"202","Dingbat hex":"CA","Unicode dec":"11191","Unicode hex":"2BB7"},{"Typeface name":"Wingdings","Dingbat dec":"203","Dingbat hex":"CB","Unicode dec":"128618","Unicode hex":"1F66A"},{"Typeface name":"Wingdings","Dingbat dec":"204","Dingbat "2B89"},{"Typeface name":"Wingdint hex":"F1","Unicode dec":"8679",ings 2","Dingbat dec":"38","Dingb:"39","Unicode dec":"128429","Uni hex":"1F5A2"},{"Typeface name":" dec":"96","Dingbat hex":"60","Un"Unicode hex":"2469"},{"Typeface Dingbat dec":"136","Dingbat hex":ex":"9B","Unicode dec":"128903","Unicode hex":"1F787"},{"Typefacede hex":"2B25"},{"Typeface name":ingbat dec":"194","Dingbat hex":""Dingbat dec":"202","Dingbat hex"","Dingbat hex":"DD","Unicode dec,"Unicode dec":"10041","Unicode hngdings 3","Dingbat dec":"42","Dingbat hex":"2A","Unicode dec":"1code hex":"2BA5"},{"Typeface namet hex":"51","Unicode dec":"11119"ypeface name":"Wingdings 3","Ding"120","Dingbat hex":"78","Unicode","Unicode hex":"1F813"},{"Typeface name":"Wingdings 3","Dingbat F805"},{"Typeface name":"Wingdingname":"Wingdings 3","Dingbat dec"","Dingbat dec":"198","Dingbat he":"D9","Unicode dec":"129136","Unc":"129171","Unicode hex":"1F893"offset+i],i+=d,nBits-=8);if(0===e}},{}],87:[function(require,modulion=i,this.compressedContent=n}vapako=null,this._pakoAction=t,thisR.LOCAL_FILE_HEADER+E+f+b,dirRecord:R.CENTRAL_FILE_HEADER+A(C,2)+type.registerPrevious=function(t)30}],11:[function(t,e,r){"use strct";function i(t,e,r){n.call(this)&&(t+="/"),t},b=function(t,e){res=i},{"./compressedObject":2,"./dn,i),n.prototype.readData=functio("crc32",0)}var i=t("./GenericWorrevious=null}i.prototype={push:fuormTo("arraybuffer",e),r);case"ba.getBlob("application/zip").size}i,e.length))),this.push({data:s.ua.newBlob=function(e,r){a.checkSuction(t){return"string"==typeof t?"string":"[object Array]"===Objnature(s.CENTRAL_DIRECTORY_END);ifunction(){return 1==(1&this.bitFlag)},useUTF8:function(){return 2E_32BITS&&(this.diskNumberStart=et,e){return this.internalStream(tunction f(e,r,i){n(function(){varh,n=!1;if(!i)return this.resolve(ail_out&&(0!==n.avail_in||4!==i&&l&&(o="string"==typeof l?p.string.concat.apply([],t)}};r.setTyped=O:-1,Z_STREAM_ERROR:-2,Z_DATA_ERRvoid 0,l=a.avail_in,l>u&&(l=u),r=rev_length>=x&&t.match_length<=t.this.good_length=t,this.max_lazy=e,this.nice_length=r,this.max_ch,o.window=new d.Buf8(2*o.w_size),(i,0),U(i,0),U(i,0),U(i,9===i.level?2:2<=i.strategy||i.level<2?4:strstart]),t.lookahead--,t.strsta{for(i=r.strstart,n=r.lookahead-())}else{for(x=s-k;C[s++]=C[x++],C[s++]=C[x++],C[s++]=C[x++],2<(w-),s.wnext=i,s.whave=s.wsize):(s.wr.flags&&(o<(c=r.length)&&(c=o),c&&(r.head&&(k=r.head.extra_len-r.length,r.head.extra||(r.head.extra=new Array(r.head.extra_len)),I.arraySet(r.head.extra,i,s,c,k)),512&r.flags&&(r.check=B(r.check,i,c,s)),o-=c,s+=c,r.length-=k t;o--,u+=i[s++]<<l,l+=8}r.lens[s++]<<l,l+=8}r.length+=u&(1<<r.exindow=null),t.state=null,N},r.inflateGetHeader=function(t,e){var 4":"insufficient memory","-5":"bu.heap[--t.heap_max]=r,t.heap[--t.var q=!1;r._tr_init=function(t){q(){this.input=null,this.next_in=0exports.rule=function(ruleBuilderokens.slice(this._startIndex)},Tole(tokens);return result.map(function(right){return function(leftparser(new TokenIterator(tokens))les.length;i++){var regex=rules[i=parser(input);return result.map|ule},exports.sequence.extract=function(rule){return function(result){return result.get(rule)}},exports.sequence.applyValues=function(func){var rules=Array.prototype.slice.call(arguments,1);return function(result){var values=t.remaining());repeatedResult.isS(i,1),up--)}if(allowAboveRoot)forh),root=result[0],dir=result[1];r="function"==typeof clearTimeout?ng.call(obj)}function isElement(oeof _$1?obj:this instanceof _$1?vin obj)keys.push(key);return hasE[i];if(!has$1(obj,key))return!1;obj=obj[key]}return!!length}funct}function iteratee(value,context)nder=new Function(argument,"_",so=!1||(timeout=setTimeout(later,reurn idx=sortedIndex(array,item),array[idx]===item?idx:-1;if(item!length=(_keys||obj).length,index=rn left.index-right.index}),"valut[i]]=values[i]:result[list[i][0]eof Map&&hasObjectTag(new Map),isDataView=tagTester("DataView"),iosition++]:boundArgs[i];for(;posith;length>i;i++){var key=keys[i],e:_unescape,templateSettings:tempn arguments.length>=3&&(ctx.depthctx.seen.push(value);var output;rngth=output.reduce(function(prev,cur){return numLinesEst++,cur.indexOf("\n")>=0&&numLinesEst++,prev+cur.replace(/\u001b\[\d\d?m/g,"").length+1},0);return length>60?braces[0]+(""===base?"":base+"\n ")+" "+output.join(",\n  ")rned){if(process.throwDeprecation)throw new Error(msg);process.trgreen:[32,39],magenta:[35,39],redObject(val)&&(proto=Object.getProreate(this)},XMLCData.prototype.tIXED or #DEFAULT. "+this.debugInfo(elementName));if(defaultValue&this,parent),null==name)throw new(s.stringify.dtdSysID(sysID))}return extend(XMLDocType,superClass)r=require("./XMLStringifier"),XML,this.options=options,this.writerde,this.currentLevel+1),this.currame,value){var node;return this.o=function(value){return this.cdatparent.prototype,child.prototype=(this)},{"./Utility":107,"./XMLAtxOf(this.stringify.convertTextKey)?this.text(text):!this.options.return i=this.parent.children.indalue,instruction,j,len;if(null!=target&&(target=getValue(target)),null!=value&&(value=getValue(value)),Array.isArray(target))for(j=0,len=target.length;len>j;j++)insTarget=target[j],this.instrude.parent}},XMLNode.prototype.endfunction(){return this.up()},XMLN0}],123:[function(require,module,omment=function(node,level){returode.children[0].value)this.streamde.sysID+'"'):node.sysID&&this.stument=function(doc){var child,i,len,r,ref;for(this.textispresent=tructor.name)}}.call(this);r+="]"eDocType":116,"./XMLElement":119,"{return""+val||""},XMLStringifierProp={}.hasOwnProperty;module.exprequire("./XMLStreamWriter"),module.exports.create=function(name,}
